...
from pubshow_en_db import get_pubshow_db, init_pubshow_db

log = logging.getLogger('pubshow')

pubshow_en_bp = Blueprint('pubshow_en', __name__, url_prefix='/pubshow-en')
...
def _email_boas_vindas(nome: str, code: str, token: str, plano: str, trial_ends: str) -> tuple:
    ...
    tv_url = f'{base_url}/pubshow-en/tv/{code}'
    jk_url = f'{base_url}/pubshow-en/jukebox/{token}'
    painel_url = f'{base_url}/pubshow-en/painel'
...
def _email_trial_ending(nome: str, plano: str, trial_ends: str) -> tuple:
    ...
    base_url = os.environ.get('BASE_URL', 'https://pubshow.com.br')
    planos_url = f'{base_url}/pubshow-en/planos'
...
def _admin_required(f):
    ...
    def decorated(*args, **kwargs):
        if not _admin_ok():
            return redirect('/pubshow-en/admin/login')
...
def pubshow_login_required(f):
    ...
    def decorated(*args, **kwargs):
        if not session.get('pub_business_id'):
            return redirect('/pubshow-en/entrar')
        return f(*args, **kwargs)
    return decorated
...
def index():
    ...
    conn.close()
    return render_template('pubshow_en/landing.html',
                           planos=PLANOS, canais=CANAIS,
                           total_videos=total_videos, total_canais=total_canais)


@pubshow_en_bp.route('/entrar', methods=['GET', 'POST'])
def entrar():
    if session.get('pub_business_id'):
        return redirect('/pubshow-en/painel')
    erro = ''
    ...
    return render_template('pubshow_en/entrar.html', erro=erro)


@pubshow_en_bp.route('/sair')
def sair():
...
@pubshow_en_bp.route('/recuperar', methods=['GET', 'POST'])
def recuperar_senha():
    msg  = ''
    erro = ''
    if request.method == 'POST':
        ...
        if not b:
            ...
            link = f'{_base}/pubshow-en/recuperar/{token}'
        ...
        conn.close()
    return render_template('pubshow_en/recuperar.html', msg=msg, erro=erro)


@pubshow_en_bp.route('/recuperar/<token>', methods=['GET', 'POST'])
def recuperar_senha_form(token):
    ...
    conn.close()
    if not b:
        return render_template('pubshow_en/recuperar.html',
    ...
    if request.method == 'POST':
        ...
        if len(nova) < 6:
            ...
            conn2.commit(); conn2.close()
            return redirect('/pubshow-en/entrar?ok=senha')
    return render_template('pubshow_en/recuperar_form.html', token=token, erro=erro)


@pubshow_en_bp.route('/bem-vindo')
...
def bem_vindo():
    ...
    if not b:
        return redirect('/pubshow-en/entrar')
    return render_template('pubshow_en/bem_vindo.html', b=dict(b), planos=PLANOS, canais=CANAIS)


@pubshow_en_bp.route('/cadastro', methods=['GET', 'POST'])
def cadastro():
    if session.get('pub_business_id'):
        return redirect('/pubshow-en/painel')
    erro = ''
            ...
            if dup_cpf:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    return redirect('/pubshow-en/bem-vindo')
                except Exception as ex:
                    if 'UNIQUE' in str(ex):
                        erro = 'Este e-mail já está cadastrado. Faça login.'
                    else:
                        erro = f'Erro ao cadastrar: {ex}'
    return render_template('pubshow_en/cadastro.html', erro=erro,
...
def tv(code):
    ...
    _jk_url   = f"https://www.4kitem.com.br/pubshow-en/jukebox/{_jk_token}"
    try:
    ...
    return render_template('pubshow_en/tv.html', b=dict(b), canal=canal,
...
@pubshow_en_bp.route('/jukebox/<token>', methods=['GET', 'POST'])
def jukebox(token):
    conn = get_pubshow_db()
    ...
    conn.close()
    if not b:
        return render_template('pubshow_en/qr_invalido.html'), 404
    if b['suspenso']:
        return render_template('pubshow_en/qr_invalido.html'), 403
    ...
    return render_template('pubshow_en/jukebox.html',
                           b=b, canal=canal,
                           tipos=precos_bar,
...
@pubshow_en_bp.route('/jukebox/<token>/ja-paguei/<int:pedido_id>', methods=['POST'])
...
@pubshow_en_bp.route('/api/status/<code>')
def api_status(code):
    conn = get_pubshow_db()
    b = conn.execute('SELECT * FROM pubshow_businesses WHERE code=?', (code,)).fetchone()
    if not b:
...
@pubshow_en_bp.route('/api/pedido-status/<token>/<int:pedido_id>')
...
@pubshow_en_bp.route('/api/pedido-exibido/<int:pedido_id>', methods=['POST'])
...
@pubshow_en_bp.route('/api/trocar-canal/<code>', methods=['POST'])
...
@pubshow_en_bp.route('/api/videos/<categoria>')
def api_videos(categoria):
    videos = _videos_do_canal(categoria)
    return jsonify({'videos': videos, 'total': len(videos)})


@pubshow_en_bp.route('/api/ranking/<code>')
...
@pubshow_en_bp.route('/api/buscar-biblioteca')
...
@pubshow_en_bp.route('/api/buscar')
def api_buscar():
...
def painel():
    ...
    return render_template('pubshow_en/painel.html',
...
@pubshow_en_bp.route('/painel/fila-json')
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/canal', methods=['POST'])
@pubshow_login_required
def painel_canal():
    b = _get_business()
    if not b: return redirect('/pubshow-en/entrar')
...
@pubshow_en_bp.route('/painel/promo', methods=['POST'])
@pubshow_login_required
def painel_promo():
...
@pubshow_en_bp.route('/painel/push/subscribe', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/push/unsubscribe', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/push/test', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/api/validar-cupom/<codigo>')
...
@pubshow_en_bp.route('/admin/cupons')
@_admin_required
def admin_cupons():
    conn = get_pubshow_db()
    ...
    conn.close()
    return render_template('pubshow_en/admin_cupons.html', cupons=cupons)


@pubshow_en_bp.route('/admin/cupons/criar', methods=['POST'])
@_admin_required
def admin_cupons_criar():
    ...
    if not codigo:
        return redirect('/pubshow-en/admin/cupons')
    conn = get_pubshow_db()
    try:
        conn.execute(
...
@pubshow_en_bp.route('/admin/cupons/toggle/<int:cid>', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/cupons/excluir/<int:cid>', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/painel/push/vapid-key')
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/toggle-slides-sistema', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/anuncios', methods=['POST'])
@pubshow_login_required
def painel_anuncios():
...
@pubshow_en_bp.route('/painel/upload-slide', methods=['POST'])
@pubshow_login_required
def painel_upload_slide():
    ...
    if total >= max_slides:
        return redirect('/pubshow-en/painel?erro=limite_slides')
    f = request.files.get('imagem')
    if not f or not f.filename:
        return redirect('/pubshow-en/painel')
    ext = f.filename.rsplit('.', 1)[-1].lower()
    if ext not in ('png', 'jpg', 'jpeg', 'gif', 'webp'):
        return redirect('/pubshow-en/painel?erro=formato_invalido')
    code = b['code']
    pasta = os.path.join(_SLIDES_DIR, code)
    os.makedirs(pasta, exist_ok=True)
    fname = f'{random.randint(10000,99999)}.{ext}'
    f.save(os.path.join(pasta, fname))
    url = f'/pubshow-en/slide/{code}/{fname}'
...
@pubshow_en_bp.route('/slide/<code>/<filename>')
...
@pubshow_en_bp.route('/_cron/emails')
def cron_emails():
...
@pubshow_en_bp.route('/_cron/sync-assinaturas')
...
@pubshow_en_bp.route('/painel/manifest.json')
...
@pubshow_en_bp.route('/sw-painel.js')
def painel_sw():
    """Serve o Service Worker do painel com escopo /pubshow-en/ permitido.
    Precisa estar em /pubshow-en/sw-painel.js (não em /static/) para que o
    browser aceite o escopo /pubshow-en/ via header Service-Worker-Allowed.
    """
    import os as _os
    ...
    resp.headers['Service-Worker-Allowed'] = '/pubshow-en/'
...
@pubshow_en_bp.route('/manifest/<token>.json')
def jukebox_manifest(token):
    ...
    manifest = {
        ...
        'start_url':        f'/pubshow-en/jukebox/{token}',
        'scope':            f'/pubshow-en/jukebox/{token}',
        'display':          'standalone',
        'background_color': '#08080f',
        'theme_color':      '#08080f',
        'orientation':      'portrait',
        'lang':             'pt-BR',
        'icons': [
            {'src': f'/pubshow-en/pwa-icon/{token}/192.png', 'sizes': '192x192', 'type': 'image/png', 'purpose': 'any maskable'},
            {'src': f'/pubshow-en/pwa-icon/{token}/512.png', 'sizes': '512x512', 'type': 'image/png', 'purpose': 'any maskable'},
        ],
        'screenshots': [],
    }
    return _Response(
...
@pubshow_en_bp.route('/pwa-icon/<token>/<int:size>.png')
...
@pubshow_en_bp.route('/painel/delete-slide', methods=['POST'])
@pubshow_login_required
def painel_delete_slide():
    ...
    if url_del.startswith('/pubshow-en/slide/'):
...
@pubshow_en_bp.route('/painel/temas', methods=['POST'])
@pubshow_login_required
def painel_temas():
...
@pubshow_en_bp.route('/painel/happy-hour', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/notif', methods=['POST'])
@pubshow_login_required
def painel_notif():
...
@pubshow_en_bp.route('/painel/pix', methods=['POST'])
@pubshow_login_required
def painel_pix():
    b = _get_business()
...
@pubshow_en_bp.route('/painel/qrcode')
@pubshow_login_required
def painel_qrcode():
    ...
    jukebox_url = f"https://www.4kitem.com.br/pubshow-en/jukebox/{token}"
    ...
    return render_template('pubshow_en/qrcode.html',
                           b=dict(b),
                           jukebox_url=jukebox_url,
                           qr_b64=qr_b64)


@pubshow_en_bp.route('/painel/pedido/<int:pid>/dispensar', methods=['POST'])
@pubshow_login_required
def painel_dispensar_pedido(pid):
    ...
    conn.commit(); conn.close()
    return redirect('/pubshow-en/painel?aba=fila')


@pubshow_en_bp.route('/painel/pedido/<int:pid>/confirmar-pix', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/pedido/<int:pid>/recusar-pix', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/toggle-pix', methods=['POST'])
@pubshow_login_required
...
@pubshow_en_bp.route('/painel/config', methods=['POST'])
@pubshow_login_required
def painel_config():
...
@pubshow_en_bp.route('/painel/senha', methods=['POST'])
@pubshow_login_required
def painel_senha():
    b = _get_business()
    if not b: return redirect('/pubshow-en/entrar')
    ...
    if not check_password_hash(b['password_hash'], atual):
        return redirect('/pubshow-en/painel?aba=conta&erro=senha_errada')
    if len(nova) < 6:
        return redirect('/pubshow-en/painel?aba=conta&erro=senha_curta')
    if nova != conf:
        return redirect('/pubshow-en/painel?aba=conta&erro=senha_diferente')
    conn = get_pubshow_db()
    conn.execute('UPDATE pubshow_businesses SET password_hash=? WHERE id=?',
                 (generate_password_hash(nova), b['id']))
    conn.commit(); conn.close()
    return redirect('/pubshow-en/painel?aba=conta&ok=senha')


@pubshow_en_bp.route('/painel/relatorio')
@pubshow_login_required
def painel_relatorio():
    ...
    conn.close()
    return render_template('pubshow_en/relatorio.html',
                           b=dict(b),
...
@pubshow_en_bp.route('/painel/novo-qr', methods=['POST'])
@pubshow_login_required
def painel_novo_qr():
    ...
    return redirect('/pubshow-en/painel/qrcode')


# ── MULTI-LOCAIS (plano Rede) ─────────────────────────────────────────────────

@pubshow_en_bp.route('/painel/rede')
@pubshow_login_required
def painel_rede():
    ...
    conn.close()
    return render_template('pubshow_en/painel_rede.html',
                           b=dict(b),
                           filiais=[dict(f) for f in filiais],
                           planos=PLANOS)


@pubshow_en_bp.route('/painel/rede/nova-filial', methods=['GET', 'POST'])
@pubshow_login_required
    ...
    if request.method == 'POST':
        ...
        if not all([nome, email, telefone, senha]):
            ...
            try:
                ...
                return redirect('/pubshow-en/painel/rede?ok=filial')
            except Exception as ex:
                if 'UNIQUE' in str(ex):
                    erro = 'Este e-mail já está cadastrado.'
                else:
                    erro = f'Erro ao cadastrar filial: {ex}'
    return render_template('pubshow_en/painel_rede_nova.html',
                           b=dict(b), erro=erro, tipos=TIPOS_ESTABELECIMENTO)


# ── ADMIN MASTER ──────────────────────────────────────────────────────────────

@pubshow_en_bp.route('/admin/login', methods=['GET', 'POST'])
def admin_login():
    erro = ''
    if request.method == 'POST':
        senha = request.form.get('senha', '')
        ...
        if admin_pass and senha == admin_pass:
            ...
            return redirect('/pubshow-en/admin')
        erro = 'Senha incorreta.'
    return render_template('pubshow_en/admin_login.html', erro=erro)


@pubshow_en_bp.route('/admin/logout')
def admin_logout():
    session.pop('pubshow_admin', None)
    return redirect('/pubshow-en/admin/login')


@pubshow_en_bp.route('/admin')
@_admin_required
def admin_dashboard():
    conn = get_pubshow_db()
    bars = conn.execute(
    ...
    conn.close()
    return render_template('pubshow_en/admin.html',
...
@pubshow_en_bp.route('/admin/bar/<int:bid>')
@_admin_required
def admin_bar(bid):
    import json as _json
    conn = get_pubshow_db()
    b = conn.execute('SELECT * FROM pubshow_businesses WHERE id=?', (bid,)).fetchone()
    if not b:
        conn.close(); return redirect('/pubshow-en/admin')
    ...
    try:
        ...
        anuncios_list = []
    return render_template('pubshow_en/admin_bar.html',
...
@pubshow_en_bp.route('/admin/bar/<int:bid>/acao', methods=['POST'])
@_admin_required
def admin_bar_acao(bid):
    ...
    conn.commit(); conn.close()
    return redirect(f'/pubshow-en/admin/bar/{bid}')


@pubshow_en_bp.route('/admin/bar/<int:bid>/upload-slide', methods=['POST'])
@_admin_required
def admin_bar_upload_slide(bid):
    ...
    f = request.files.get('imagem')
    if not f or not f.filename:
        conn.close(); return redirect(f'/pubshow-en/admin/bar/{bid}')
    ext = f.filename.rsplit('.', 1)[-1].lower()
    if ext not in ('png', 'jpg', 'jpeg', 'gif', 'webp'):
        conn.close(); return redirect(f'/pubshow-en/admin/bar/{bid}?erro=formato')
    code = b['code']
    pasta = os.path.join(_SLIDES_DIR, code)
    os.makedirs(pasta, exist_ok=True)
...
@pubshow_en_bp.route('/admin/slides-sistema')
@_admin_required
def admin_slides_sistema():
    ...
    conn.close()
    return render_template('pubshow_en/admin_slides_sistema.html',
                           slides=[dict(s) for s in slides])


@pubshow_en_bp.route('/admin/slides-sistema/add', methods=['POST'])
@_admin_required
def admin_slides_sistema_add():
    ...
    if titulo:
        ...
        conn.commit(); conn.close()
    return redirect('/pubshow-en/admin/slides-sistema')


@pubshow_en_bp.route('/admin/slides-sistema/upload', methods=['POST'])
@_admin_required
def admin_slides_sistema_upload():
    ...
    ext = f.filename.rsplit('.', 1)[-1].lower()
    if ext not in ('png', 'jpg', 'jpeg', 'gif', 'webp'):
        return redirect('/pubshow-en/admin/slides-sistema?erro=formato')
    pasta = os.path.join(_SLIDES_DIR, 'sistema')
    os.makedirs(pasta, exist_ok=True)
    fname = f'sis_{random.randint(100000, 999999)}.{ext}'
    f.save(os.path.join(pasta, fname))
    url = f'/pubshow-en/slide-sistema/{fname}'
    conn = get_pubshow_db()
    conn.execute(
...
@pubshow_en_bp.route('/slide-sistema/<filename>')
...
@pubshow_en_bp.route('/admin/slides-sistema/toggle/<int:sid>', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/slides-sistema/delete/<int:sid>', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos')
@_admin_required
def admin_videos():
    conn = get_pubshow_db()
    videos = conn.execute(
    ...
    return render_template('pubshow_en/admin_videos.html',
                           por_nicho=por_nicho,
                           total=len(videos),
                           canais_grupos=canais_grupos)


@pubshow_en_bp.route('/admin/import-playlist', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/check-one', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/check-bulk', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/remover-quebrados', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/deletar', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/add', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/add-batch', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/oembed', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/admin/videos/seed-categorias', methods=['POST'])
@_admin_required
...
@pubshow_en_bp.route('/assinar/<plano>', methods=['GET', 'POST'])
@pubshow_login_required
def assinar(plano):
        ...
        try:
            ...
            if not customer_id:
                ...
                if sub.get('id'):
                    ...
                    return redirect('/pubshow-en/aguardando')
                else:
    ...
    return render_template('pubshow_en/checkout.html', b=dict(b), plano=plano, p=PLANOS[plano], erro=erro)


@pubshow_en_bp.route('/aguardando')
@pubshow_login_required
def aguardando():
    b = _get_business()
    return render_template('pubshow_en/aguardando.html', b=dict(b))


@pubshow_en_bp.route('/painel/cancelar', methods=['GET', 'POST'])
@pubshow_login_required
def painel_cancelar():
    ...
    if request.method == 'POST':
        ...
        if confirmacao != 'CANCELAR':
            return render_template('pubshow_en/cancelar.html', b=bd, ass=dict(ass) if ass else None,
        ...
        return redirect('/pubshow-en/painel?ok=cancelamento')

    return render_template('pubshow_en/cancelar.html', b=bd, ass=dict(ass) if ass else None, erro='')


@pubshow_en_bp.route('/planos')
def planos():
    return render_template('pubshow_en/planos.html', planos=PLANOS, canais=CANAIS)
        ...
        if tipo == 'pagamento_confirmado':
            ...
            corpo = f"""
              ...
              <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
                <a href="{base_url}/pubshow-en/painel" style="display:inline-block;background:#4ade80;color:#08080f;font-weight:800;font-size:15px;padding:12px 32px;border-radius:8px;text-decoration:none;">
                  Ir para o Painel →
                </a>
              </td></tr></table>
            """
...
@pubshow_en_bp.route('/webhook/asaas', methods=['GET', 'POST'])
def webhook_asaas():
    """Recebe eventos de assinatura do Asaas.

    Configurar no painel Asaas → Configurações → Webhooks:
      URL: https://<dominio>/pubshow-en/webhook/asaas
