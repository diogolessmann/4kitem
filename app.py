...
def _security_headers(resp):
    ...
    # ── Bolha WhatsApp flutuante nas páginas públicas (1 ponto cobre todas as landings) ──
    # Denylist: não polui fluxos de app/checkout/admin. No mobile sobe p/ não colidir
    # com as barras CTA sticky de rodapé (VetZap/DefesaPro/MandaZap).
    # Bloqueia por SEGMENTO (em qualquer nível): pega /somaja/painel, /vetzap/pix etc.
    # Nomes escolhidos p/ NÃO colidir com landings de produto (ex.: 'agenda' é produto).
    _WA_DENY_SEG = {'api', 'webhook', 'saas-admin', 'admin', 'painel', 'app',
                    'checkout', 'pix', 'assinar', 'assinatura', 'cadastro',
                    'cadastrar', 'entrar', 'login', 'sair', 'conversa', 'conversas',
                    'paciente', 'pacientes', 'relatorio', 'evolucao', 'sinais',
                    'consentimento', 'redefinir', 'esqueci', 'dev', 'health'}
    try:
        p = request.path
        ...
        if (request.method == 'GET' and resp.status_code == 200
            ...
            if '</body>' in body and 'wa-float-4k' not in body:
                bubble = (
                    '<style>#wa-float-4k{position:fixed;bottom:20px;right:20px;z-index:9999;'
                    ...
                    '<path d="M.057 24l1.687-6.163a11.867 11.867 0 01-1.587-5.945C.16 5.335 '
                    '5.495 0 12.05 0a11.82 11.82 0 018.413 3.488 11.82 11.82 0 013.48 8.414c-.003 '
                    '6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 01-5.688-1.448L.057 24zm6.597-3.807c1.676.995 '
                    '3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 '
                    '0-9.887 4.434-9.889 9.884a9.86 9.86 0 001.516 5.26l-.999 3.648 3.972-.99zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.767.967-.94 '
                    ...
                    '4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>'
                    '</svg></a>')
                resp.set_data(body.replace('</body>', bubble + '</body>', 1))
    except Exception:
        pass
    return resp
